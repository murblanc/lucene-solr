...
public class AddReplicaCmd implements CollectionHandlingUtils.Cmd {
  ...
  private ModifiableSolrParams getReplicaParams(ClusterState clusterState, ZkNodeProps message, @SuppressWarnings({"rawtypes"})NamedList results, String collectionName, DocCollection coll, boolean skipCreateReplicaInClusterState, String asyncId, ShardHandler shardHandler, CreateReplica createReplica) throws IOException, InterruptedException, KeeperException {
    ...
    if (!skipCreateReplicaInClusterState) {
      ...
      if (ccc.getDistributedClusterChangeUpdater().isDistributedStateChange()) {
        ccc.getDistributedClusterChangeUpdater().doSingleStateUpdate(DistributedClusterChangeUpdater.MutatingCommand.SliceAddReplica, props,
    ...
    params.set(CoreAdminParams.CORE_NODE_NAME,
        CollectionHandlingUtils.waitToSeeReplicasInState(ccc.getZkStateReader(), ccc.getSolrCloudManager().getTimeSource(), collectionName, Collections.singletonList(createReplica.coreName)).get(createReplica.coreName).getName());
