...
public class SplitShardCmd implements CollectionHandlingUtils.Cmd {
    ...
    try {
      ...
      for (int i = 0; i < subRanges.size(); i++) {
        ...
        if (ccc.getDistributedClusterChangeUpdater().isDistributedStateChange()) {
          ccc.getDistributedClusterChangeUpdater().doSingleStateUpdate(DistributedClusterChangeUpdater.MutatingCommand.CollectionCreateShard, new ZkNodeProps(propMap),
      ...
      if (hasRecordedDistributedUpdate && ccc.getDistributedClusterChangeUpdater().isDistributedStateChange()) {
      ...
      if (leaderZnodeStat == null || ephemeralOwner != leaderZnodeStat.getEphemeralOwner()) {
        ...
        ZkNodeProps m = new ZkNodeProps(propMap);
        if (ccc.getDistributedClusterChangeUpdater().isDistributedStateChange()) {
          ccc.getDistributedClusterChangeUpdater().doSingleStateUpdate(DistributedClusterChangeUpdater.MutatingCommand.SliceUpdateShardState, m,
              ccc.getSolrCloudManager(), ccc.getZkStateReader());
        } else {
