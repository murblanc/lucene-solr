...
public class OverseerCollectionMessageHandler implements OverseerMessageHandler, SolrCloseable {
  ...
  final Map<CollectionAction, CollectionHandlingUtils.Cmd> commandMap;
  ...
  public OverseerCollectionMessageHandler(ZkStateReader zkStateReader, String myId,
    ...
    commandMap = new ImmutableMap.Builder<CollectionAction, CollectionHandlingUtils.Cmd>()
        ...
        .put(MOCK_COLL_TASK, new CollectionHandlingUtils.MockOperationCmd())
        .put(MOCK_SHARD_TASK, new CollectionHandlingUtils.MockOperationCmd())
        .put(MOCK_REPLICA_TASK, new CollectionHandlingUtils.MockOperationCmd())
        .put(CREATESHARD, new CreateShardCmd(ccc))
        .put(MIGRATE, new MigrateCmd(ccc))
        .put(CREATE, new CreateCollectionCmd(ccc))
        .put(MODIFYCOLLECTION, new CollectionHandlingUtils.ModifyCollectionCmd(ccc))
        .put(ADDREPLICAPROP, new CollectionHandlingUtils.AddReplicaPropCmd(ccc))
        .put(DELETEREPLICAPROP, new CollectionHandlingUtils.DeleteReplicaPropCmd(ccc))
        .put(BALANCESHARDUNIQUE, new CollectionHandlingUtils.BalanceShardsUniqueCmd(ccc))
        .put(REBALANCELEADERS, new CollectionHandlingUtils.RebalanceLeadersCmd(ccc))
        .put(RELOAD, new CollectionHandlingUtils.ReloadCollectionCmd(ccc))
  ...
  public OverseerSolrResponse processMessage(ZkNodeProps message, String operation) {
    ...
    try {
      ...
      CollectionHandlingUtils.Cmd command = commandMap.get(action);
      if (command != null) {
