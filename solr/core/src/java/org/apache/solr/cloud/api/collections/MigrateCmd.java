...
public class MigrateCmd implements CollectionHandlingUtils.Cmd {
  ...
  private void migrateKey(ClusterState clusterState, DocCollection sourceCollection, Slice sourceSlice,
    ...
    if (ccc.getDistributedClusterChangeUpdater().isDistributedStateChange()) {
      ccc.getDistributedClusterChangeUpdater().doSingleStateUpdate(DistributedClusterChangeUpdater.MutatingCommand.SliceAddRoutingRule, m,
    ...
    coreNodeName = CollectionHandlingUtils.waitForCoreNodeName(tempSourceCollectionName, targetLeader.getNodeName(),
        tempCollectionReplica2, ccc.getZkStateReader());
    // wait for the replicas to be seen as active on temp source leader
    if (log.isInfoEnabled()) {
